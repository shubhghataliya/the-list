...
export default function ImportExport({ onImport, onExport, onTextImport }: ImportExportProps) {
  ...
  return (
    <div className="mt-8 pt-5 border-t border-zinc-800/60">
      ...
      <button
        ...
        className="w-full flex items-center justify-center gap-2 bg-violet-500/10 hover:bg-violet-500/15 border border-violet-500/20 hover:border-violet-500/30 text-violet-400 hover:text-violet-300 rounded-xl py-2.5 text-sm font-medium transition-all duration-200 active:scale-[0.98]"
      >
        <List className="w-4 h-4" />
        Import from Numbered List
      </button>
    </div>
  );
}
